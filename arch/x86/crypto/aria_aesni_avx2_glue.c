...
static DEFINE_PER_CPU(u8 [ARIA_AESNI_AVX2_PARALLEL_BLOCK_SIZE], keystream);
...
static int aria_avx2_ctr_encrypt(struct skcipher_request *req)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		u8 _ks[ARIA_BLOCK_SIZE];
		u8 *ks;

		while (nbytes >= ARIA_AESNI_AVX2_PARALLEL_BLOCK_SIZE) {
			kernel_fpu_begin();
			ks = this_cpu_ptr(keystream);
			aria_ops.aria_ctr_crypt_32way(ctx, dst, src, &ks[0],
						      walk.iv);
			kernel_fpu_end();
		...
		while (nbytes >= ARIA_AESNI_PARALLEL_BLOCK_SIZE) {
			kernel_fpu_begin();
			ks = this_cpu_ptr(keystream);
			aria_ops.aria_ctr_crypt_16way(ctx, dst, src, &ks[0],
						      walk.iv);
			kernel_fpu_end();
		...
		while (nbytes >= ARIA_BLOCK_SIZE) {
			memcpy(&_ks[0], walk.iv, ARIA_BLOCK_SIZE);
			crypto_inc(walk.iv, ARIA_BLOCK_SIZE);

			aria_encrypt(ctx, &_ks[0], &_ks[0]);

			crypto_xor_cpy(dst, src, &_ks[0], ARIA_BLOCK_SIZE);
			dst += ARIA_BLOCK_SIZE;
			src += ARIA_BLOCK_SIZE;
		...
		if (walk.nbytes == walk.total && nbytes > 0) {
			...
			crypto_xor_cpy(dst, src, &_ks[0], nbytes);
			dst += nbytes;
			src += nbytes;
			nbytes = 0;
		}
